...
inner join departments t2
on t1.department_id=t2.department_id
where t2.department_name in("IT","IT Support","Treasury");
...
where t3.city = "Seattle" or t3.city = "Toronto";
...
-- Вывести имя и фамилию сотрудника и имя и фамилию его менеджера * (self join)
select
t1.first_name as employee_first_name,
t1.last_name as employee_last_name,
t2.first_name as manager_first_name,
t2.last_name as manager_first_name
from employees t1 
inner join  employees t2 
on t1.manager_id = t2.employee_id;

-- Вывести job_id тех сотрудников, которые зарабатывают больше своего менеджера * (self join)
select distinct
t1.job_id
from employees t1 
inner join employees t2 
